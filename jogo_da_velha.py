...
_____|_____|_____
  ...
  7  |  8  |  9
    ...
    print('Você quer ser o X (xis) ou a O (bola)?... digite sair (para sair) ', end=' ')
    while j != 'O' and j != 'X' and j != 'sair' or j != '"sair"':
        j = str(input('Digite X ou O e pressione Enter para escolher... digite sair (para sair): ')).strip().upper()
        if j != 'O' and j != 'X' and j != 'sair' or j != '"sair"':
            print('\nEscolha inválida!\n')
    if j == 'O':
        adv = 'X'
...
_____|_____|_____
  ...
  {}  |  {}  |  {}
    ...
    if reiniciar == 's' or reiniciar == '"s"':
        ...
        def menu_jogos():
            while True:
                jogar_novamente = input("\n Deseja retornar ao menu de jogos? (s/n) ")
                if jogar_novamente.lower() == "s" or jogar_novamente == '"n"':
                    import jogos
                    jogos.escolha_jogo()
                    break
                elif jogar_novamente.lower() == "n" or jogar_novamente == '"n"':
